...
def listAvailablePdf():
    ...
    for file in os.listdir(papersDir):
        if file.endswith(".pdf"):
            s += '"' + file.replace(".pdf", "") + '",'
            count += 1
            # filePath = papersDirWin + "\\" + file
            # imgPath = papersDirWin + "\\img\\" + file.replace(".pdf",".png")
            # os.system(r"ConvertPDFtoImage.EXE /S "+filePath+" /T \""+imgPath+"\" /C1  /1 1 /5 10")
